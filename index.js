...
function createDictionary(entries) {
  ...
  entries.forEach(([key, value]) => dictionary.set(key, value));
  return dictionary;
...
function convertMapToObject(map) {
  ...
  for (const [key, value] of map) {
    ...
    obj = Object.fromEntries(dictionary);
  }
  return obj;

  // map.forEach(([key, value]) => dictionary.set(key, value));

  //  let item = Object.fromEntries(map);
  // let res = map.entries(item);
  //console.log(res);
...
function addKeyValuePairs(dictionary, entries) {
  ...
  entries.forEach(([key, value]) => {
    ...
    dictionary.set(`added`, added).set(`rejected`, rejected);
  });

  return Object.fromEntries(dictionary, added, rejected);
  //console.log(` added: ${added}, rejected: ${rejected}`);
...
function transformDictionary(dictionary) {
  ...
  return { keys: keysArr, values: valuesArr, entries: entriesArr };
...
function checkPresence(dictionary, keys) {
  ...
  keys.forEach((elem) => {
    if (dictionary.has(elem)) {
      return presence.push(true);
    } else {
      return presence.push(false);
    }
  });
  return presence;
...
function getFilteredDictionarySize(dictionary, filter) {
  ...
  return ` Лише ${res.size} міста мають населення більше ніж 1 мільйон `;
...
function resetDictionary(dictionary, maxSize) {
  if (dictionary.size >= maxSize) {
    dictionary.clear();
    console.log(`Cловник був очищений`);
    return true;
  }
  console.log(
    `Кількість елементів не перевищує максимально допустиму, тому словник не був очищений`
  );
  return false;
